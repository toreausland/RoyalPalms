"""
Oppstart-script for Royal Palms Kjøpergruppe.
Kjøres automatisk ved deploy (startCommand).

Håndterer:
1. Automatisk backup av databasen før endringer
2. Database-migrasjoner (Flask-Migrate / Alembic)
3. Seeding av data (admin, inviterte brukere, temaer)
"""
import os
import shutil
...
def backup_database(app):
    """Tar backup av databasen hvis den eksisterer."""
    db_uri = app.config['SQLALCHEMY_DATABASE_URI']
    if not db_uri.startswith('sqlite:///'):
        return  # Bare SQLite støttes
    ...
    if os.path.exists(db_path):
        ...
        # Behold kun de 3 siste backupene
        backup_dir = os.path.dirname(db_path)
        backups = sorted([
            f for f in os.listdir(backup_dir)
            if f.startswith('royal_palms.db.backup_')
        ])
        while len(backups) > 3:
            old = backups.pop(0)
            os.remove(os.path.join(backup_dir, old))
            print(f'  Gammel backup slettet: {old}')


def run_migrations(app):
    """Kjører database-migrasjoner, håndterer førstegangsoppsett."""
    migrations_dir = os.path.join(os.path.dirname(__file__), 'migrations')

    if not os.path.exists(migrations_dir):
        # Ingen migrasjoner funnet — bruk create_all som fallback
        db.create_all()
        print('Database opprettet med db.create_all() (ingen migrasjoner funnet).')
        return
    ...
    try:
        upgrade()
        print('Database-migrasjoner kjørt.')
    except Exception as e:
        if 'already exists' in str(e):
            # Eksisterende database uten migrasjonshistorikk —
            # stamp den som «ferdig migrert» til nåværende versjon
            stamp(revision='head')
            print('Eksisterende database stemplet med migrasjonsversjon.')
        else:
            raise


def seed_data():
    """Seeder database med admin, inviterte brukere og temaer. Idempotent."""
